...
# Check if 'timestamp' column exists, and if not, add it
c.execute('''PRAGMA table_info(tasks)''')
columns = c.fetchall()
column_names = [column[1] for column in columns]
if 'timestamp' not in column_names:
    c.execute('''ALTER TABLE tasks ADD COLUMN timestamp DATETIME DEFAULT CURRENT_TIMESTAMP''')
    conn.commit()
...
def show_today_tasks():
    ...
    if tasks:
        ...
        for task in tasks:
            table.add_row(str(task[0]), task[1], task[2], task[3])
        console = Console()
        console.print(f"Tasks for {today_date}:")
        console.print(table)
    else:
        console = Console()
        console.print(f"No tasks for {today_date}")
